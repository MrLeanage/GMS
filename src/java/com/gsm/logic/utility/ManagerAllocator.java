 ...
 */
package com.gsm.logic.utility;

import com.gsm.data.model.Grievance;
import com.gsm.data.model.GrievanceManagerStatistics;
...
public class ManagerAllocator {
    ...
    public static String getFreeGrievanceManager() {
        ...
        for (User user : userArrayList) {
            if (user.getuType().equals("Manager - Level 3")) {
                l3Managers.add(user.getuEmpID());
            }
        }
    ...
    public static String getFreeHighLevelManager(String grivanceID, String type){
        GrievanceService grievanceService = new GrievanceService();
        UserService userService = new UserService();
        ...
        for(Grievance grievance :processingGrievances){
          for(User user : grievanceService.getGrievanceChatParticipants(grievance.getgID())){
              if(user.getuType().equals(type)){
                  processingGrievanceChatUsers.add(user.getuEmpID());
              }
          }
        }
        ...
        for(User user :userService.loadAllUserByPosition(type, "Active")){
            grievanceManagerStatisticsList.add(new GrievanceManagerStatistics(user.getuEmpID(), 0, 0, 0, 0) );
        }
        ...
        for(User user : existingGrievanceManagers){
            processingGrievanceChatUsers.removeIf(element -> element.equals(user.getuEmpID()));
        }
